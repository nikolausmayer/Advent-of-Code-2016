...
fn main(){
    ...
    let mut full_distances = BTreeMap::<(i32, i32), i32>::new();
    {
        let from_key = 0;
        full_distances.insert((from_key, from_key), 0);
        for to_key in &keys {
            if &from_key >= to_key {
                continue;
            }
            ...
            while let node = heap.pop() {
                ...
                let node = node.unwrap();
                if node.heuristic == 0 {
                    full_distances.insert((from_key, *to_key), node.cost);
                    full_distances.insert((*to_key, from_key), node.cost);
                    break;
                }
                if visited.contains(&(node.x, node.y)) {
                    continue;
    ...
    fn recurser (keys_left : Vec<i32>, last_key : i32, dist : i32, mut mindist : &mut i32, distances : BTreeMap<(i32, i32), i32>, full_distances : &BTreeMap<(i32, i32), i32>) {
        if keys_left.len() == 0 {
            if dist + full_distances[&(last_key, 0)] < *mindist {
                *mindist = dist + full_distances[&(last_key, 0)];
            }
        } else {
            for i in 0..keys_left.len() {
                ...
                recurser(rest, next_key, dist + to_next, mindist, newdist, full_distances);
            }
        }
    }
    ...
    recurser(keys_to_get, 0, 0, &mut mindist, newdist, &full_distances);
    println!("{}", mindist);
}
